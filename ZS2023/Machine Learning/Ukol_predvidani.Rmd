...
c("RMSE" = sqrt(mean((predpoved_hdd - data_HDD_test_2018$HDD_SO)^2)))
```
...
c("RMSE" = sqrt(mean((predpoved_hdd_2019 - data_HDD_test_2019$HDD_SO)^2)))
```
...
c("RMSE" = sqrt(mean((predpoved_hdd_2020 - data_HDD_test_2020$HDD_SO)^2)))
```
...
lines(model_vekMatky$fitted.values)
...
lines(model_Rozvodovost$fitted.values)
...
lines(model_nezamestnanost$fitted.values)
...
lines(model_Plodnost$fitted.values)
